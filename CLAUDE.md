...
| Detalle | `gui/dashboard_detalle.py` | Per-cylinder detail chart (click a cylinder in Vista Real). The "Distribución de Diámetros (Activos)" histogram paints **each SubStock's diameter zone** `[hasta, desde]` (`_pintar_zonas_substock`): a faint full-height tint over the bars + labeled `J{n}` strips stacked in **lanes** above the histogram. Since SubStock bands **may overlap**, lanes are assigned by *lane packing* (non-overlapping bands share a lane, overlapping ones get separate stacked lanes), so overlaps are visible without one zone hiding another; colors come from `JAULA_COLORS` (`config/tema.py`) |
