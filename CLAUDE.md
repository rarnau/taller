...
**Two utilization KPIs** (`modelos/kpis.py`, the single source consumed by the KPIs tab, CLI and dashboard): **disponible** = `tiempo_total_ocupada_min / tiempo_calendario` (key `utilizacion_maquinas_pct`, unchanged) and **neta** = `tiempo_total_ocupada_min / tiempo_operativo` (key `utilizacion_neta_pct`, where operative time = `minutos_operativos_entre(snapshots[0], snapshots[-1])`). They are **equal under 24/7** (operative == calendar) and `neta ≥ disponible` otherwise. Only `utilizacion_neta_pct` was added, so the golden fingerprint (which reads only `utilizacion_maquinas_pct`) is unchanged. The Dashboard utilization bar shows the **disponible** metric (its title says so) and the Gantt shades each machine's closed-shift spans in `RED_DARK` (`_tramos_parada_maquina`).
