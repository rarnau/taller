...
| Monte Carlo | `gui_qt/montecarlo_qt.py` (`MonteCarloPanel`) | Param-range sliders + option chips (selection/assignment/replenishment strategies, generator, laminador shifts) + per-machine priority chips and shift selectors (custom grids via the shared `TurnosDialog` pop-up, stored compact) + N runs; runs the sweep in background (`MonteCarloService`), **updates cards/histograms/table progressively every chunk (~10%)**, and manages the **result set** (incremental CSV + spec sidecar): pause (clean cancel), resume/extend, and “open set” restoring the input ranges from the sidecar (see "Monte Carlo runner") |
...
### Defined weighted assignment strategy (prioridad_j1_j4_piso10) + unique-attribution stock metric
A second shipped `EstrategiaAsignacion` — **`_PrioridadJ1J4ConPiso` ("prioridad_j1_j4_piso10")** — encodes a fixed stock-prioritization policy **inside the strategy** (deliberately NOT per-cage config): class constants `PISO_STOCK = 10` and `PESOS = {1: 1.44, 2: 1.2, 3: 1.0, 4: 1.2}` (cages beyond 4 weigh 1.0). Selection precedence among the diameter-admissible candidates: (1) stopped cages first; (2) **safety floor** — any candidate whose stock is below `PISO_STOCK` beats every at-or-above-floor one, and among below-floor candidates the emptiest wins (raw stock, weights ignored); (3) at/above the floor, weighted balance: minimize `stock/peso` (a cage of peso 1.2 targets 20% more stock than a 1.0 one — J1 20% over J2, J2/J4 20% over J3); (4) tie by cage number. The stock compared is **`TallerCilindros.stock_activos_por_jaula()`**: every non-BAJA cylinder attributed to **exactly one** cage via `_jaula_atribuida` — TRABAJANDO/CRC → its physical cage, else `jaula_destino` if set, else the **lowest-numbered** band containing its current diameter, else key `0` ("sin banda") — so the values always sum to the total active count even with overlapping bands. Like every assignment strategy it only has effect when bands overlap; the default strategy is unchanged, so golden KPIs are identical.

**The Análisis evolution chart uses the same unique-attribution metric.** `generar_snapshot()` fills the new `Snapshot.activos_por_jaula` (`{jaula: n}`, all cages seeded 0, key `0` only when present) inside its **single pass**; `gui_qt/analysis_data.py` feeds the card "Evolución de stock por jaula (activos)" (`gui_qt/analysis_qt.py`) from it, adding a grey "Sin banda" series only if key `0` ever has cylinders — the chart total now equals the active-cylinder count instead of double-counting overlap cylinders per band (`disponibles_por_substock` keeps its old semantics for Vista Real). **Adding `activos_por_jaula` to `Snapshot` changed `snapshots_sha256`** (KPIs/n_snapshots/alerts untouched), so the golden master was regenerated on purpose (precedent: `detalle_maquinas_operativa`). Not in `CAMPOS_SNAPSHOT_KPI` (light snapshots skip it). Guarded by `tests/test_estrategia_ponderada.py`.
...
**Lightweight snapshot mode (`TallerCilindros.snapshot_ligero`, for Monte Carlo).** The KPIs (`modelos/kpis.py`) read only three snapshot fields — `tiempo`, `jaulas_paradas`, `cantidad_disponibles` — declared **once** in `CAMPOS_SNAPSHOT_KPI` (`modelos/eventos.py`, next to `Snapshot`; it can't live in `kpis.py` because `taller.py` must not import `kpis` — import cycle). With `snapshot_ligero = True` (a plain bool instance attribute, default `False`; a **runner** decision, never persisted to `user_config.json`), `generar_snapshot()` still emits one snapshot per event (same `len(snapshots)`) but computes **only** the registered fields — it iterates `CAMPOS_SNAPSHOT_KPI` and dispatches each field to its compute block in `TallerCilindros._BLOQUES_SNAPSHOT_KPI` — skipping the SubStock loop, the three `detalle_maquinas*` dicts (per-machine `esta_operativa`/`en_falla` per event are not free), `detalle_jaulas`/`detalle_crc`/`crc_por_jaula`, the queue/cooling detail lists and `disponibles_por_substock`; unregistered fields keep their `Snapshot.__init__` empties. **Rule: adding a KPI that reads another snapshot field ⇒ add the field to `CAMPOS_SNAPSHOT_KPI` and its compute block to `_BLOQUES_SNAPSHOT_KPI`** — a registered field without a block fails loudly (`KeyError` on the first light snapshot), and `tests/test_snapshot_ligero.py` asserts registry ⇄ blocks match 1:1, KPI equivalence full vs light (`metricas_montecarlo` + scalar `calcular_kpis` on a PARADA scenario) and that heavy fields stay at their empties. Default `False` is byte-identical to the historical full mode (golden untouched; the GUI, CLI `simular` and `batch_simular` always run full). ~2.8x faster `simular()` on the 140-cylinder week.
