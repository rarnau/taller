...
| Generación de Cambios | `gui/tab_generacion.py` | Self-contained synthetic-change workflow: generator **config** (algorithm, desbaste threshold, horizon, laminador shift regime — moved here out of Configuración), **adaptation** (upload history → refit incremental or from scratch + persisted-model info), **generation** (reproducible seed) and a Matplotlib **timeline** of generated changes per jaula shading the line-stopped (PARADA) spans (reuses `dashboard_principal._marcar_paradas`). Like `tab_config` it reads/writes `app.user_cfg` and drives `app.taller`; "Generar cambios" builds the taller from the stock + synthetic `Programa_Cambios` and triggers `App._simular()`, which calls back `gen_widget.refrescar_timeline()` so the PARADA bands appear |
