...
`MonteCarloPanel` is a thin **orchestrator**: its UI components are isolated in the **`gui_qt/widgets/montecarlo/`** subpackage — `FloatSlider`, `HistogramWidget`, `RangeRow` (coupled Mín/Máx sliders), `ChipSelector` (toggle chips over a hidden combo, `changed` signal; `orientation="flow"` lays text-width chips that wrap via `FlowChips` — a self-contained container that repositions chips and pins its own height in `resizeEvent`, avoiding the fragile `heightForWidth` path — else one/row-expanded), `ShiftSelector` (per-machine turnos incl. the "Personalizado" grid, flow-wrapped chips, lazy-imports `TurnosDialog` to avoid the import cycle), `MachineCard` (a `SectionCard` owning one machine's rate/falla `RangeRow`s + priority `ChipSelector` + `ShiftSelector`, exposing `rangos()/prioridad()/turnos()` getters+setters), `ResultsView` (right panel: KPI cards + histograms + summary table + export signals) and `formatting` (`KPI_DESTACADOS`, `fmt_mc_kpi`). The panel only assembles these, arms the spec (`_mc_desde_widgets`/`_aplicar_mc_a_widgets`) and drives the run/set lifecycle. This subpackage is **not** re-exported from `gui_qt/widgets/__init__.py` (that would cycle: `widgets` → `montecarlo` → `config_qt` → `widgets`); import from `gui_qt.widgets.montecarlo` directly.
