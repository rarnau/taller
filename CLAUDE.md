...
> **⚠️ The suite depends on `config/user_config.json` being the pristine defaults.** Every golden scenario (and several unit tests in `test_turnos.py`/`test_fallas.py`) builds its config with `cargar_config()`, which reads the repo's `config/user_config.json`. If a personalized config gets committed (machine `turnos`, `tasa_falla > 0`, `tiempo_enfriado_h > 0`, `turnos_cambios`, …) the engine behavior changes and the golden tests fail even though the engine code is untouched. This is the current state on `main`: the "Add files via upload" commit replaced the JSON with a real user config and ~11 tests fail. Fix by restoring the default JSON (see `DEFAULTS` in `config/persistencia.py`) or by pointing the tests at a fixture config instead of the live user file.
